...
socket.on('disconnection', async (player) => {
    if (document.querySelector(`#trivia-game-${player.id}`)) {
        exit.play();
        await animateCSS(`#trivia-game-${player.id}`, 'fadeOutUp');
        document.querySelector(`#trivia-game-${player.id}`).remove();
    }
});
socket.on('startGame', showQuestionsArea);
