...
function appendMessage({ name = '', message, id }, { correctGuess = false, closeGuess = false, lastWord = false } = {}) {
    ...
    if (correctGuess) {
        document.getElementById(`trivia-game-${id}`).classList.add('correct');
...
socket.on('updateScore', ({
    ...
    document.querySelector(`#trivia-game-${playerID}>div p:last-child`).textContent = `Score: ${score}`;
    document.querySelector(`#trivia-game-${drawerID}>div p:last-child`).textContent = `Score: ${drawerScore}`;
});
